...
`attributedict` uses **setuptools** with a `src/` layout.
...
(`dynamic = ["version"]`) — the setuptools equivalent of the
...
target 3.9), producing a `cp39-abi3` wheel per platform.
...
`cibuildwheel` builds abi3 wheels for :
...
Releases are a manual, protected step. No publishing
